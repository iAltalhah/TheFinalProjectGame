...
namespace AllIn1ShaderNodes
{
	public class RenderPipelineChecker : AssetPostprocessor
	{
		...
		private const string INCLUDE_HELPER_FILE_PATH = "Assets/Plugins/AllIn1ShaderNodes/Shaders/AllIn1Node_IncludeHelper.hlsl";
		...
		public static void CreateIncludeHelper()
		{
		...
		private static void EnsureValidIncludeHelperFile()
		{
			bool fileExists = File.Exists(INCLUDE_HELPER_FILE_PATH);

			if (fileExists)
			{
				string fileContent = File.ReadAllText(INCLUDE_HELPER_FILE_PATH);

				bool isValid = false;
				switch (CurrentRenderPipeline)
				{
					case RenderPipeline.BIRP:
						isValid = fileContent.Contains("ALLIN1NODE_BIRP");
						break;
					case RenderPipeline.URP:
						isValid = fileContent.Contains("ALLIN1NODE_URP");
						break;
					case RenderPipeline.HDRP:
						isValid = fileContent.Contains("ALLIN1NODE_HDRP");
						break;
				}
