...
namespace AllIn1ShaderNodes
{
    ...
    {
        private const string Version = "1.141";
        ...
        private void OnGUI()
        {
            ...
            using(var scrollView = new EditorGUILayout.ScrollViewScope(scrollPosition, GUILayout.Width(position.width), GUILayout.Height(position.height)))
            {
				...
				GUILayout.Space(10);
				DrawLine(Color.grey, 1, 3);
                RefreshRenderPipelineConfigurationSection();
        ...
        private void RefreshRenderPipelineConfigurationSection()
        {
			GUILayout.Label("Render Pipeline Configuration", bigLabel);
			GUILayout.Space(10);

			if (GUILayout.Button("Refresh Configuration", GUILayout.MaxWidth(BUTTON_WIDTH)))
            {
                RenderPipelineChecker.CreateIncludeHelper();
			}
		}
